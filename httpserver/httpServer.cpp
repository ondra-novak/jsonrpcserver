...
public:
	LiveLogListener(ILogOutput::LogType logLevel,PNetworkStream connection, bool asEventStream)
		:LiveLogStream(logLevel),connection(connection), asEventStream(asEventStream) {}

	class IOp: public RefCntObj {
	public:
...
inline natural LiveLogHandler::onRequest(IHttpRequest& request,ConstStrA vpath) {
	...
	if (levelDesc == ConstStrA("fatal")) t = ILogOutput::logFatal;
	else if (levelDesc == ConstStrA("error")) t = ILogOutput::logError;
	else if (levelDesc == ConstStrA("warning")) t = ILogOutput::logWarning;
	else if (levelDesc == ConstStrA("note"))   t = ILogOutput::logNote;
	else if (levelDesc == ConstStrA("progress"))  t = ILogOutput::logProgress;
	else if (levelDesc == ConstStrA("info"))  t = ILogOutput::logInfo;
    else if (levelDesc == ConstStrA("debug"))  t = ILogOutput::logDebug;
...
inline natural LiveLogHandler::onData(IHttpRequest& request) {
	return 0;
}
